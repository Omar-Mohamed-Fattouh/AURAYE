...
export default function AboutVision() {
  ...
  return (
    <section className="relative min-h-screen bg-black text-white overflow-y-auto py-16 flex items-center justify-center px-4">
      {/* subtle gradient / glow background */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,_#ffffff33,_transparent_55%),radial-gradient(circle_at_bottom,_#88888833,_transparent_55%)] opacity-70" />

      <div className="relative container mx-auto grid grid-cols-1 md:grid-cols-2 justify-center items-start gap-12 px-6 lg:px-10">
        {/* LEFT SIDE */}
        <LeftTextSide />
...
function LeftTextSide() {
  return (
    <div className="space-y-8 max-w-xl mx-auto md:mx-0">
      ...
      <motion.p
        className="text-sm sm:text-base md:text-base text-white/70 leading-relaxed"
        initial={{ opacity: 0, y: 16 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        <Link
          to="/tryon"
          className="inline-flex items-center gap-2 rounded-full bg-white text-black text-sm font-semibold px-6 py-2.5 hover:bg-gray-200 transition-colors"
        >
  ...
  return (
    ...
    >
      <button
        ...
        transition-colors duration-300 ${isOpen ? "text-white" : "text-white/60"}`}
      >
        <div className="flex items-center gap-3">
        ...
        <div className="overflow-hidden">
          <div className="mt-2 mb-4 flex flex-col md:flex-row gap-4 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-3">
            ...
            <div className="flex-1 flex flex-col justify-between gap-2">
              ...
              <Link
                to="/tryon"
                className="inline-flex w-max items-center gap-2 text-xs font-medium text-white hover:text-gray-300"
              >
...
function VerticalLineAndGlowingDot({ activeId }) {
  const step = 100 / (aboutAccordionContent.length + 0.5);
  ...
  return (
    <>
      ...
      <span
        className="pointer-events-none absolute left-0 top-3 w-px bg-gradient-to-b from-white via-gray-400 to-transparent transition-all duration-500 ease-in-out"
        style={{ height: `${topOffset}%` }}
      />

      <span
        className="pointer-events-none absolute left-[-5px] w-3 h-3 rounded-full bg-white shadow-[0_0_18px_rgba(255,255,255,0.9)] transition-all duration-500 ease-in-out"
