...
import GlassesSizingSection from "../components/ClassSizingSection";
...
const ProductID = () => {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const [pRes, allRes, bestRes, reviewsRes] = await Promise.all([
        getProductById(id),
        getProducts(),
        getBestSellerProducts(),
      ...
      setReviews(reviewsRes?.data ?? reviewsRes ?? []); // <-- SET REVIEWS
    } catch (e) {
  ...
  if (loading)
    return <AurayeLoader label="Loading product" subtitle="AURAYE" />;
  ...
  return (
    <div>
      ...
      <ProductReviews
        reviews={reviews}
        productId={id}
        user={currentUser} // pass your logged user info here
      />
      <GlassesSizingSection productImage={product.images[0].url} />
